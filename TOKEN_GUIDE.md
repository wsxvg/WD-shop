...
### 7.1 关注列表用的是哪两个接口
都在 `wdbuybuy` 域，**POST**，**需要 token**（`context` 参数）：

| 接口路径 | 作用 |
|---|---|
| `https://thor.weidian.com/wdbuybuy/maimai.getFollowShops/2.0` | 我关注的**全部**店铺（分页，pageNum/pageSize） |
| `https://thor.weidian.com/wdbuybuy/maimai.getFollowShopsHavingShelfItems/2.0` | 我关注的店铺里**当前有在售商品**的那部分 |

请求体（form-urlencoded）两个字段：
```
param   = {"pageNum":1,"pageSize":50}      # JSON 字符串
context = {...你的 token JSON...}          # JSON 字符串（就是登录态）
```
返回 `result.dataList[]` 是店铺列表，`result.hasMore` 表示还有没有下一页。
（对应代码见 `crawl.py` 的 `crawl()` 函数与 `main()` 阶段2/阶段3。）

> 单店的商品用的是另一个**公开接口** `decorate/shopDetail.tab.getItemList/1.0`（GET，不要 token），所以商品刷新不受 token 影响。

### 7.2 更新关注列表 = 更新 token 后本地跑一次
关注列表**只能靠 token 拉取**。你在微店里新关注/取关了店铺，要让本商城同步，步骤是：

1. 抓包导出 HAR（打开「我关注的微店」页面时抓）；
2. 本地执行：
   ```powershell
   python extract_token.py xxx.har   # 写入 data/context.json
   python crawl.py                   # 用新 token 拉最新关注列表 + 全量商品
   python build_page.py              # 重新生成 index.html
   git add data/follow_shops.json data/status.json index.html
   git commit -m "更新关注列表"
   git push
   ```
3. 推送后 GitHub Pages 自动更新，大家看到的就是最新关注名单。

### 7.3 为什么 GitHub Actions 自动爬取不会更新关注列表
GitHub 上**没有你的 token**（`context.json` 被 `.gitignore` 挡住、也绝不该上传）。所以每小时的自动爬取**只会刷新已有店铺的商品**（公开接口），**不会发现新关注的店**。这是安全设计，不是 bug。

要让"关注列表"也自动更新，有两条路（二选一）：

- **推荐·手动**：按 7.2 每隔一段时间本地跑一次并 push（token 一般能活几周到几个月）。安全，不泄露账号。
- **进阶·自动**：把 token 存成 GitHub Secret 注入 Actions（方便但有风险——一旦仓库或日志泄露，token 等于账号密码外泄）。如需这么做再单独找 AI 配置，务必清楚风险。
